...
func CmdBackup(databaseName string, skipPoll bool, id IDb, is services.IServices, ij jobs.IJobs) error {
	...
	// TODO Uncomment and delete the below version after the restore command is uncommented
	...
	logrus.Printf("You can download your backup with the \"datica db download %s %s ./output_file_path\" command. Backups expire after 30 days. Please note that if your service is deleted, so will all backup jobs associated with that service.", databaseName, job.ID)
	return nil
}
